...
- [x] Создать БД, выполнить миграцию
- [x] Загрузить `backend/` на хостинг через FTP/FileZilla
- [x] Заполнить `config.php` реальными данными
- [x] Прописать реальный `API_BASE` в `web/js/api.js`
- [x] Протестировать все endpoints через Postman/curl
