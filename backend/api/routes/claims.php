...
if ($method === 'POST' && !$id) {
    ...
    // Используем транзакцию чтобы исключить двойное взятие (race condition)
    $db->beginTransaction();
    try {
...
if ($method === 'PUT' && $id) {
    ...
    if ($role === 'child' && $action === 'submit') {
        $cid = $auth['sub'];
        $s = $db->prepare("UPDATE task_claims SET status='pending',submitted_at=NOW() WHERE id=? AND child_id=? AND status='in_progress'");
        $s->execute([$id, $cid]);
        if ($s->rowCount() === 0) Response::error('Claim не найден');
        Notify::taskSubmitted($db, (int)$id, $fid);
        Response::success(['submitted' => true]);
    }

    if ($role === 'parent' && in_array($action, ['approve','reject'])) {
        $s = $db->prepare('SELECT tc.*,t.coins_reward,t.one_time_claim FROM task_claims tc JOIN tasks t ON t.id=tc.task_id WHERE tc.id=? AND t.family_id=? AND tc.status="pending"');
        $s->execute([$id, $fid]);
        $cl = $s->fetch();
        ...
        if ($action === 'approve') {
            ...
            // Скрыть одноразовое задание после выполнения
